		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-12 align-self-center p-static text-center mt-2">
					<h1 class="font-weight-bold text-color-secondary text-11">Projects</h1>
				</div>
			</div>
		</div>
	</section>
					...
					<div class="card custom-card border-radius-0 appear-animation" data-appear-animation="fadeIn" data-appear-animation-delay="600">
						<div class="card-body text-center py-3">
							<div class="feature-box-icon d-flex justify-content-center w-auto my-2 py-4" style="height: 240px">
								<img  width="240px" src="{{asset('storage/'.($project->image ?? ''))}}" alt="" data-plugin-options="{'color': '#220c3c', 'animated': true, 'delay': 0, 'strokeBased': true}" />
							</div>
							<h3 class="card-title mb-3 text-5 text-secondary font-weight-bold">{{$project->title}}</h3>
							@if ( $project->project_status == 'Running')
							<p class="card-text text-warning">{{$project->project_status}}</p>
							@else 
							<p class="card-text text-success">{{$project->project_status}}</p>
							@endif
